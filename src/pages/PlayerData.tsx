...
/**
 * Boss trophies for the first SEVEN Forsaken — used to derive bosses-defeated, because the
 * save file's own boss_kills counter is unreliable.
 *
 * Valheim 1.0 added an eighth: Kall Fimbulbringer (`ach_boss8frozenking`, "Defeat the
 * Shackled One"). He is deliberately NOT in this set — **he drops no trophy at all.**
 * Every other Deep North creature has one (Moose, Seal, Barka, Writhan, Elaking, both
 * Jotun), but the boss himself does not, so a trophy count can never see him.
 */
 ...
 *
 * He drops Sacrificial Blood (`FrozenKingDrop`) and a Crown Jewel, and the game fires a
 * Hugin tutorial whose label is literally "Hugin: Kall Fimbulbringer defeated"
 * (`tutorial_sacrificialblood_*`). Known texts persist on the character, whereas the blood
 * itself is consumable — it is "the final key" — so the tutorial is the durable signal and
 * the items are only a fallback for a character who still happens to be carrying one.
 *
 * Matching is substring + case-insensitive on purpose: the exact known-text key could not
 * be confirmed against a real save (nobody had killed him yet when this was written), so
 * this errs towards detecting rather than missing him.
 */
...
const TOTAL_BOSSES = BOSS_TROPHY_IDS.size + 1;
...
export function PlayerData() {
  ...
  // Trophy-derived boss count (save-file boss_kills is unreliable), plus Kall
  // Fimbulbringer, who has to be detected separately because he drops no trophy.
                  ...
                  {filteredKnowledge.slice(0, 200).map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-1.5 px-2">
                          <span className="text-sm text-zinc-200 font-medium mr-1.5">{item.name}</span>
                          <button
                            type="button"
                            onClick={(e) => copyItemName(item, e)}
                            title={`Copy "${item.name}"`}
                            className="inline-flex p-1 rounded text-zinc-600 hover:text-brand-400 hover:bg-zinc-800/60 transition-colors align-middle"
                          >
